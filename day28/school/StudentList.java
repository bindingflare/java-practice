...
public class StudentList {
	...
	public List<Student> searchByStudent (String query) {
		...
		for(Student student : stdArr) {
			if(student.getName().contains(query) || student.getID().contains(query)) {
	...
	public boolean edit(Student student) {
		...
		if(!isRemoved) {
			StdManager.log("Edit failed! Student with name " + student.getName() + " and ID " + student.getID() + " does not exist in student list");
			return false;
		}
